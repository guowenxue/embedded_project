...
#if 1  /*Add by guowenxue to support FB command*/
...
/* This part add by guowenxue 2009.07.08, which used to check the image validation. */		

    /* The image is not image 0(bootloader information), but linux kernel or romfs image */
	if(0 < image_footer->num) 
	{
	   /* First 4 bytes is the GHL magic number: 'GHLS' in big end 'SLHG'(0x534C4847),
	    * which added by "mkimage" tools */
	   if(0x534C4847 != *(UINT32 *)image_source) 
	   {
		  uprintf("Check Image Validation:\t\t Failure!\n");
		  return -1;
	   }
	   uprintf("Check Image Validation:\t\t OK.\n");	
	
	   // Skip the 64 bytes GHL head information 
	   image_footer->length -= sizeof(image_header_t);   // 64 bytes
	   image_source += sizeof(image_header_t);           // 64 bytes
	}
/* Add end */

	if( (image_footer->base & (FLASH_BLOCK_SIZE-1)) )
	{
		uprintf("Burn to flash address is not block alignment.\n");
...
/* This part add by guowenxue 2009.07.08, which used to check the image validation. */	
	// First 4 bytes is the GHL magic number: 'GHLS' in big end ('SLHG'),which added by "mkimage" tools.
	if(0x534C4847 != *(UINT32 *)image_source) 
	{
		uprintf("Check Image Validation:\t\t Failure!\n");
		return -1;
	...
	// Skip the 64 bytes GHL head information 
	image_footer->length -= sizeof(image_header_t);
	image_source += sizeof(image_header_t);
/* Add end */	
