...
// forward action routine definitions
 ...
 int FB_action(int argc, char *argv[]);
...
INT FB_usage()
{
	uprintf("Usage: FB [Src Addr] [Dst Addr] [Image Size]\n");
	uprintf("Src Addr:  The raw image(from)  memory address\n");
	uprintf("Dst Addr:  The raw image burned to flash address\n");
	uprintf("Image Size:  The raw image size\n");
	uprintf("Declare:  This command can used to burn bootloader to flash directly.\n");
	return -1;
}
...
int FB_action(int argc, char *argv[])
{
	...
	srcAddr = s2hex(argv[1]);
	dstAddr = s2hex(argv[2]);
	fileSize=s2dec(argv[3]);

	uprintf("Burn raw image from 0x%08x(mem) to 0x%08x(flash) with size %d \n ", srcAddr, dstAddr, fileSize );

	if( 0==flash_raw_write((void *)srcAddr, (void *)dstAddr, fileSize) )
	{
		uprintf("Bootloader update successed!\n");
		return 0;
	}
	else
	{
		uprintf("ERROR: Bootloader update failure!\n");
		return -1;
	}

	return 0; /*Don't come here to avoid compiler complain*/
}
